...
// Daemonize if not debug mode
  ...
  meas=new TMeasH(&a,2,19200); // create new object
//meas->start(QThread::NormalPriority);
  ...
  int writeHWReq=0;
  if(meas->localeSt){ // program in locale mode one time write data
    printf("Start in locale mode\n");
    while(1) { a.processEvents(); sleep(1); }
  }
  else
    while(1){ // program in remoute control mode
      a.processEvents();
